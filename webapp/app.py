...
#boom_data = boom_data_all.ix[:5000]
#with open('js/boom100.json', 'w') as geojson_file:
    #geojson_file
    #geojson_file.write('collisions = [')
    #json.dump(geo_boom, geojson_file)
    #geojson_file.write('];')
geo_boom = FeatureCollection([geojson.Feature(geometry=geojson.Point((p_long, p_lat)), properties={"NUMBER OF PERSONS INJURED": n_inj, 
                                                                                 "NUMBER OF PERSONS KILLED": n_kill})
            for p_long, p_lat, n_inj, n_kill in zip(boom_data['LONGITUDE'], boom_data['LATITUDE'], 
                                                    boom_data['NUMBER OF PERSONS INJURED'], boom_data['NUMBER OF PERSONS KILLED'])])
...
@app.route("/geojson_data", methods=["GET", "POST"])
def get_geojson_data():   
    return geojson.dumps(geo_boom)
